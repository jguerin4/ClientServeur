...
public class BanqueClient extends JApplet implements ActionListener {
	...
	public void init() {
		...
		try {
			...
			idAjouterSommeTitle = new TextField(
					"ID du compte à mettre à jour:", 20);
			...
			retirerSomme = new Button(
					"      Retirer de l'argent à un compte      ");
		...
		if (evt.getSource() == CreerCompteButton) {
			try {
				...
				if (isInteger(idField.getText())
					...
					etatApplet
							.setText("Compte créé avec succès avec les valeurs suivantes: "
									+ "\n"
									+ "ID: "
									+ nouveauId
									+ "\n"
					...
					etatApplet
							.setText("Transaction annulé, veuillez recommencer avec des formats d'entrées valides.");
			...
			try {
				...
				if (isInteger(idDetailCompte.getText())) {
					...
					etatApplet.setText("");
					etatApplet
							.setText("Voici les détails du compte: "
									+ "\n"
									+ "ID: "
			...
			try {
				...
				if (isInteger(idAjouterSommeCompte.getText()) // Vérifie le
																// format des
																// données
					...
					if (remoteReference.ajoutSomme(idAjoutSomme,
						...
						etatApplet
								.setText("La transaction à été annulé, un problème est survenu.");
					}
				}
			...
			try {
				...
				if (isInteger(idRetirerSommeCompte.getText()) // Vérifie le
																// format des
																// données
	...
	public static boolean isInteger(String s) { // Code basé sur ce lien
